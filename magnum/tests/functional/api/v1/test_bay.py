...
class BayTest(base.BaseMagnumTest):
    ...
    @testtools.testcase.attr('positive')
    def test_list_bays_empty(self):
        resp, model = bay_cli.BayClient.as_user('default').list_bays()
        self.assertEqual(resp.status, 200)
        self.assertEqual(len(model.bays), 0)
    ...
    @testtools.testcase.attr('positive')
    def test_create_bay_with_existing_name(self):
        model = datagen.valid_bay_data(baymodel_id=BayTest.baymodel.uuid,
                                       name='test')
        _, first_model = self._create_bay(model, user='default')
        _, second_model = self._create_bay(model, user='default')
        self.assertNotEqual(first_model.uuid, second_model.uuid)
    ...
    @testtools.testcase.attr('negative')
    def test_create_bay_times_out(self):
        gen_model = datagen.valid_bay_data(baymodel_id=BayTest.baymodel.uuid,
                                           node_count=40, bay_create_timeout=1)
        self._create_invalid_bay(gen_model, 'default')

    @testtools.testcase.attr('positive')
    def test_create_swarm_bay_with_multiple_masters(self):
        gen_model = datagen.valid_bay_data(baymodel_id=BayTest.baymodel.uuid,
                                           master_count=2)
        _, old_model = self._create_bay(gen_model, user='default')
        resp, model = bay_cli.BayClient.as_user('default').get_bay(
            old_model.uuid)

        self.assertEqual(200, resp.status)
        self.assertEqual(model.master_count, 2)
    ...
    @testtools.testcase.attr('positive')
    def test_update_bay_node_count_for_existing_bay(self):
        first_model = datagen.valid_bay_data(baymodel_id=BayTest.baymodel.uuid,
                                             name='test')
        _, old_model = self._create_bay(first_model, user='default')

        patch_model = datagen.bay_node_count_patch_data()
        bay_client = bay_cli.BayClient.as_user('default')
        ...
        self.assertEqual(model.node_count, new_model.node_count)
    ...
    @testtools.testcase.attr('positive')
    def test_update_bay_with_to_existing_name(self):
        first_model = datagen.valid_bay_data(baymodel_id=BayTest.baymodel.uuid,
                                             name='test')
        _, old_model = self._create_bay(first_model, user='default')
