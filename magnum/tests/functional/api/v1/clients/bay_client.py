...
class BayClient(client.ClientMixin):
    ...
    def wait_for_bay_to_delete(self, bay_id):
        utils.wait_for_condition(
            lambda: self.does_bay_not_exist(bay_id), 1, 300)

    def wait_for_created_bay(self, bay_id):
        utils.wait_for_condition(
            lambda: self.does_bay_exist(bay_id), 1, 300)

    def wait_for_final_state(self, bay_id):
        utils.wait_for_condition(
            lambda: self.is_bay_in_final_state(bay_id), 1, 300)

    def is_bay_in_final_state(self, bay_id):
        try:
            resp, model = self.get_bay(bay_id)
            if model.status in ['CREATED', 'CREATE_COMPLETE', 'ERROR', 'CREATE_FAILED']:
                return True
            else:
                return False
        ...
        try:
            ...
            if model.status in ['CREATED', 'CREATE_COMPLETE']:
                ...
                raise exceptions.ServerFault(
                    "Got into an error condition: %s" % model)
            else:
                return False
        except exceptions.NotFound:
            return False
