...
def gen_docker_volume_size(min_int=1, max_int=10):
...
def baymodel_data(name=data_utils.rand_name('bay'),
                  keypair_id=data_utils.rand_name('bay'),
                  image_id=data_utils.rand_name('bay'),
                  flavor_id=data_utils.rand_name('bay'), public=False,
                  master_flavor_id=data_utils.rand_name('bay'), labels=None,
                  dns_nameserver=gen_random_ip(), tls_disabled=False,
                  external_network_id=str(data_utils.rand_uuid()), coe="swarm",
                  apiserver_port=gen_random_port(), fixed_network="public",
                  ssh_authorized_key=gen_fake_ssh_pubkey(),
                  docker_volume_size=gen_docker_volume_size(),
                  cluster_distro=data_utils.rand_name('bay'),
                  http_proxy=gen_http_proxy(), https_proxy=gen_https_proxy(),
                  no_proxy=gen_no_proxy(), network_driver=None):
...
def valid_swarm_baymodel(tls_disabled=False):
    """Generates a valid swarm baymodel with valid data

    :returns: BayModelEntity with generated data
    """

    return baymodel_data(image_id=config.Config.image_id, fixed_network=None,
                         ...
                         docker_volume_size=None, cluster_distro=None,
...
def invalid_swarm_baymodel():
    """Generates an invalid swarm baymodel with invalid data
    ...
    return baymodel_data(image_id=config.Config.image_id, fixed_network=None,
                         flavor_id=config.Config.flavor_id, public=False,
                         dns_nameserver="8.8.8.8", master_flavor_id="someflavorid",
                         keypair_id=config.Config.keypair_id, coe="swarm",
                         docker_volume_size=None, cluster_distro=None,
                         ssh_authorized_key=None, external_network_id="private",
                         http_proxy=None, https_proxy=None, no_proxy=None,
                         network_driver=None, labels={}, tls_disabled=False)
